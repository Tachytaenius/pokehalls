StdScripts::
	dba PokeCenterNurseScript ; CONQUERED
...
._check
	ld a, "@"
	ld hl, wEmilyTraumaString
	ld bc, 4
	call ByteFill

	ld hl, wEmilyTraumaString
	ld de, EmilyTraumaLevel
	lb bc, PRINTNUM_RIGHTALIGN | 1, 3
	call PrintNum

	ld a, "@"
	ld hl, wEmilyHealthString
	ld bc, 4
	call ByteFill

	ld hl, wEmilyHealthString
	ld de, EmilyHealthLevel
	lb bc, PRINTNUM_RIGHTALIGN | 1, 3
	call PrintNum
...
.EmilyLevels
	text "Trauma: @"
	text_from_ram wEmilyTraumaString
	text "."
	line "Vitality: @"
	text_from_ram wEmilyHealthString
	text "."
	prompt
.swap
	callasm .isRunning
	iftrue running
	callasm .getDirection
	if_equal OW_UP, .up
	if_equal OW_DOWN, .down
	if_equal OW_LEFT, .left
;	if_equal OW_RIGHT, .right
;.right
	applymovement PLAYER, ._right
	applymovement2 ._left
...
._right
	...
	step_end
.up
	applymovement PLAYER, ._up
	applymovement2 ._down
	follownotexact PLAYER, 2
	end
._up
	step_up
...
._down
	...
	step_end
.left
	applymovement PLAYER, ._left
	applymovement2 ._right
	follownotexact PLAYER, 2
	end
._left
	step_left
...
InitializeEventsScript:
	callasm .InitializeEmilyHealth
	setevent EVENT_106
	setevent EVENT_INITIALIZED_EVENTS
	return
.InitializeEmilyHealth
	ld a, 255
	ld [EmilyHealthLevel], a
	ret
